...
class Trader:
    def __init__(self, params=None):
        ...
        self.orchids_data = {"curr_edge": self.params[Product.ORCHIDS]["init_make_edge"], "volume_history": [], "optimized": False}
    ...
    def orchids_adap_edge(
        ...
        if timestamp == 0:
            self.orchids_data["curr_edge"] = self.params[Product.ORCHIDS]["init_make_edge"]
            return self.params[Product.ORCHIDS]["init_make_edge"]
        curr_edge = self.orchids_data['curr_edge']
        # Timestamp not 0
    ...
    def orchids_arb_make(
        ...
        print(f"ALGO_BID: {bid}")
        print(f"ALGO_ASK: {ask}")
              

        buy_quantity = position_limit - (position + buy_order_volume)
        if buy_quantity > 0:
            orders.append(Order(Product.ORCHIDS, aggressive_ask - 1, buy_quantity))  # Buy order
    ...
    def run(self, state: TradingState):
        ...
        if Product.ORCHIDS in self.params and Product.ORCHIDS in state.order_depths:
            ...
            orchids_position = 0
            foreign_ask = state.observations.conversionObservations[Product.ORCHIDS].askPrice
            ask = round(foreign_ask) - 3
            print(f"ALGO_ASK: {round(ask)}")
            result[Product.ORCHIDS] = [Order(Product.ORCHIDS, round(ask), 100), Order(Product.ORCHIDS, round(ask) + 1, -100)]
