...
class Trader:
    ...
    def orchids_arb_take(
        ...
        best_ask = min(order_depth.sell_orders.keys())
        aggressive_ask = 0.3939575 * observation.askPrice + 0.56392057 * implied_ask + 0.02446401 * best_ask + 18.190356238319737
    ...
    def spread_orders(self, order_depths: Dict[str, OrderDepth], product: Product, basket_position: int, spread_history: List[float]):
        ...
        spread_mean = (np.sum(spread_history) + (self.params[Product.SPREAD]["spread_mean"] * self.params[Product.SPREAD]["starting_its"] )) / ( self.params[Product.SPREAD]["starting_its"] + len(spread_history))
    ...
    def run(self, state: TradingState):
        ...
        spread_orders = self.spread_orders(state.order_depths, Product.GIFT_BASKET, basket_position, traderObject[Product.SPREAD]["spread_history"])
