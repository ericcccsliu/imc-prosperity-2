...
class Trader:
    ...
    def spread_orders(self, order_depths: Dict[str, OrderDepth], product: Product, basket_position: int, spread_data: Dict[str, Any]):
        ...
        spread_data["curr_mean"] = (spread_data['curr_mean'] * spread_data['curr_its'] + spread) / (spread_data['curr_its'] + 1)
        spread_data["curr_its"] += 1
        if len(spread_data["spread_history"]) > self.params[Product.SPREAD]["spread_std_window"]:
            spread_data["spread_history"].pop(0)
        
        spread_std = np.std(spread_data["spread_history"])
        zscore = (spread - spread_data["curr_mean"]) / spread_std
    ...
    def run(self, state: TradingState):
        ...
        if Product.SPREAD not in traderObject:
            traderObject[Product.SPREAD] = {
                "spread_history": [],
                "curr_mean": self.params[Product.SPREAD]["spread_mean"],
                "curr_its":self.params[Product.SPREAD]["starting_its"],
                "prev_zscore": 0,
                "clear_flag": False
            }
