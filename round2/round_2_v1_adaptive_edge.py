...
class Trader:
    def __init__(self, params=None):
        ...
        self.orchids_data = {"curr_edge": self.params[Product.ORCHIDS]["init_make_edge"], "volume_history": [], "prev_implied": {"bid": None, "ask": None}, "optimized": False}
    ...
    def orchids_implied_bid_ask(
        ...
        if observation.exportTariff - observation.transportFees - 0.1 != self.orchids_data["prev_implied"]["bid"] or observation.importTariff + observation.transportFees != self.orchids_data["prev_implied"]["ask"]:
            self.orchids_data["prev_implied"] = {"bid": observation.exportTariff - observation.transportFees - 0.1, "ask": observation.importTariff + observation.transportFees}
            self.orchids_data["optimized"] = False
    ...
    def orchids_adap_edge(
        ...
        if timestamp == 0:
            self.orchids_data["curr_edge"] = self.params[Product.ORCHIDS]["init_edge"]
            return self.params[Product.ORCHIDS]["init_edge"]
    
        # Timestamp not 0
        self.orchids_data["volume_history"].append(abs(position))
        if len(self.orchids_data["volume_history"]) > self.params[Product.ORCHIDS]["volume_avg_timestamp"]:
        ...
        if len(self.orchids_data["volume_history"]) < self.params[Product.ORCHIDS]["volume_avg_timestamp"]:
            ...
            if volume_avg >= self.params[Product.ORCHIDS]["volume_bar"]:
                ...
                self.orchids_data["curr_edge"] = curr_edge + self.params[Product.ORCHIDS]["step_size"]
            ...
            # Decrement edge if more cash with less edge, included discount
                ...
                if curr_edge - self.params[Product.ORCHIDS]["step_size"] > self.params[Product.ORCHIDS]["min_edge"]:
                    ...
                    self.orchids_data["curr_edge"] = self.params[Product.ORCHIDS]["min_edge"]
    ...
    def orchids_arb_make(
        ...
        # ask = observation.askPrice - 2

        # best_bid = min(order_depth.buy_orders.keys())
        # best_ask = max(order_depth.sell_orders.keys())

        buy_quantity = min(position_limit - (buy_order_volume), quote_volume)
        if buy_quantity > 0:
            orders.append(Order(Product.ORCHIDS, round(bid), buy_quantity))  # Buy order

        sell_quantity = min(position_limit - (sell_order_volume), quote_volume)
        if sell_quantity > 0:
    ...
    def run(self, state: TradingState):
        ...
        if state.traderData != None and state.traderData != "":
            ...
            self.orchids_data = traderObject[Product.ORCHIDS]
        ...
        if Product.ORCHIDS in self.params and Product.ORCHIDS in state.order_depths:
            ...
            orchids_make_orders, _, _ = self.orchids_arb_make(
                ...
                edge,
                orchids_position,
                self.params[Product.ORCHIDS]["quote_volume"],
                buy_order_volume,
                sell_order_volume
            )

            traderObject[Product.ORCHIDS] = self.orchids_data
            print(f"CURR EDGE: {self.orchids_data['curr_edge']}")
            print(f"VOLUME HISTORY: {self.orchids_data['volume_history']}")
