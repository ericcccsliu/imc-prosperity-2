...
class Trader:
    ...
    def orchids_arb_make(
        ...
        # ask = observation.askPrice - 2.1

        best_bid = min(order_depth.buy_orders.keys())
        best_ask = max(order_depth.sell_orders.keys())

        local_mid = (best_bid + best_ask) / 2
        ask_1 = round(local_mid) - 1
        ask_2 = round(local_mid) - 2

        buy_quantity = min(position_limit - (position + buy_order_volume), position_limit)
        if buy_quantity > 0:
            orders.append(Order(Product.ORCHIDS, round(bid), buy_quantity))  # Buy order

        sell_quantity = min(position_limit + (position - sell_order_volume), position_limit)
        sq0 = round(sell_quantity*0.1)
        sq1 = round(sell_quantity*0.4)
        sq2 = round(sell_quantity*0.5)
        if sell_quantity > 0:
            # orders.append(Order(Product.ORCHIDS, round(ask), -sell_quantity))  # Sell order
            orders.append(Order(Product.ORCHIDS, round(local_mid), -sq0))
            orders.append(Order(Product.ORCHIDS, round(ask_1), -sq1))
            orders.append(Order(Product.ORCHIDS, round(ask_2), -sq2))
