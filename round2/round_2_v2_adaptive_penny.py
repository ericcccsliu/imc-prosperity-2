...
class Trader:
    ...
    def orchids_arb_make(
        ...
        if best_ask - dist_to_best_level > ask:
            ask = best_ask - dist_to_best_level
        
        if best_bid + dist_to_best_level > bid:
            bid = best_bid + dist_to_best_level

        buy_quantity = min(position_limit - (position + buy_order_volume), 50)
        if buy_quantity > 0:
            orders.append(Order(Product.ORCHIDS, round(bid), buy_quantity))  # Buy order

        sell_quantity = min(position_limit + (position - sell_order_volume), 50)
        if sell_quantity > 0:
    ...
    def run(self, state: TradingState):
        ...
        if Product.ORCHIDS in self.params and Product.ORCHIDS in state.order_depths:
            ...
            orchids_make_orders, _, _ = self.orchids_arb_make(
                ...
                self.params[Product.ORCHIDS]["dist_to_best_level"],
                orchids_position,
                buy_order_volume,
                sell_order_volume
            )
