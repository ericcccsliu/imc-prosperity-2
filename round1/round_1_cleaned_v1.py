...
class Trader:
    ...
    def make_amethyst_orders(self, order_depth: OrderDepth, fair_value: int, position: int, buy_order_volume: int, sell_order_volume: int) -> (List[Order], int, int):
        orders: List[Order] = []
    ...
    def run(self, state: TradingState):
        ...
        if Product.AMETHYSTS in state.order_depths:
            ...
            amethyst_take_orders, buy_order_volume, sell_order_volume = self.take_orders(Product.AMETHYSTS, state.order_depths[Product.AMETHYSTS], amethyst_fair_value, 0, amethyst_position)
            ...
            amethyst_make_orders, _, _ = self.make_amethyst_orders(state.order_depths[Product.AMETHYSTS], amethyst_fair_value, amethyst_position, buy_order_volume, sell_order_volume)
        ...
        if Product.STARFRUIT in state.order_depths:
            ...
            starfruit_take_orders, buy_order_volume, sell_order_volume = self.take_orders(Product.STARFRUIT, state.order_depths[Product.STARFRUIT], starfruit_fair_value, starfruit_take_width, starfruit_position)
