...
class Trader:
    ...
    def coconut_coupon_orders(
        ...
        if (
            volatility
            >= self.params[Product.COCONUT_COUPON]["mean_volatility"]
            + self.params[Product.COCONUT_COUPON]["threshold"]
        ):
            if coconut_coupon_position != -self.LIMIT[Product.COCONUT_COUPON]:
                ...
                if len(coconut_coupon_order_depth.buy_orders) > 0:
                    best_bid = max(coconut_coupon_order_depth.buy_orders.keys())
                    quantity = min(
                        abs(target_coconut_coupon_position - coconut_coupon_position),
    ...
    def run(self, state: TradingState):
        ...
        if Product.ORCHIDS in self.params and Product.ORCHIDS in state.order_depths:
            ...
            orchids_take_orders, buy_order_volume, sell_order_volume = ( # TODO FIX BUY SELL ORDER VOLUME 
        ...
        if (
            ...
            if coconut_coupon_orders != None:
                ...
                target_coconut_quantity = -int(delta * net_coconut_coupon_quantity)
                if (
                    abs(coconut_position) == self.LIMIT[Product.COCONUT]
                    or target_coconut_quantity == coconut_position
                ):
