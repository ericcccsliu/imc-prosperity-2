...
class Trader:
    ...
    def get_coconut_coupon_mid_price(
        ...
        if len(coconut_coupon_order_depth.buy_orders) > 0 and len(coconut_coupon_order_depth.sell_orders) > 0:
    ...
    def delta_hedge_coconut_position(
        ...
        hedge_quantity = target_coconut_position - (coconut_position + coconut_buy_orders - coconut_sell_orders)

        orders: List[Order] = []
        if hedge_quantity > 0:
            # Buy COCONUT
            best_ask = min(coconut_order_depth.sell_orders.keys())
            quantity = min(abs(hedge_quantity), -coconut_order_depth.sell_orders[best_ask])
            quantity = min(quantity, self.LIMIT[Product.COCONUT] - (coconut_position + coconut_buy_orders))
            if quantity > 0:
            ...
            quantity = min(abs(hedge_quantity), coconut_order_depth.buy_orders[best_bid])
            quantity = min(quantity, self.LIMIT[Product.COCONUT] + (coconut_position - coconut_sell_orders))
            if quantity > 0:
    ...
    def delta_hedge_coconut_coupon_orders(
        ...
        if target_coconut_quantity > 0:
            # Buy COCONUT
            best_ask = min(coconut_order_depth.sell_orders.keys())
            quantity = min(abs(target_coconut_quantity), -coconut_order_depth.sell_orders[best_ask])
            quantity = min(quantity, self.LIMIT[Product.COCONUT] - (coconut_position + coconut_buy_orders))
            if quantity > 0:
                orders.append(Order(Product.COCONUT, best_ask, quantity))
            ...
            # Sell COCONUT
            best_bid = max(coconut_order_depth.buy_orders.keys())
            quantity = min(abs(target_coconut_quantity), coconut_order_depth.buy_orders[best_bid])
            quantity = min(quantity, self.LIMIT[Product.COCONUT] + (coconut_position - coconut_sell_orders))
            if quantity > 0:
                orders.append(Order(Product.COCONUT, best_bid, -quantity))
    ...
    def coconut_coupon_orders(
            ...
            if volatility > self.params[Product.COCONUT_COUPON]["mean_volatility"] + self.params[Product.COCONUT_COUPON]["threshold"]:
                if coconut_coupon_position != -self.LIMIT[Product.COCONUT_COUPON]:
                   ...
                   if len(coconut_coupon_order_depth.buy_orders) > 0:
                       best_bid = max(coconut_coupon_order_depth.buy_orders.keys())
                       quantity = min(abs(target_coconut_coupon_position - coconut_coupon_position), abs(coconut_coupon_order_depth.buy_orders[best_bid]))
                       traderData["hedged"] = False
                       return [Order(Product.COCONUT_COUPON, best_bid, -quantity)]

            elif volatility < self.params[Product.COCONUT_COUPON]["mean_volatility"] - self.params[Product.COCONUT_COUPON]["threshold"]:
                if coconut_coupon_position != self.LIMIT[Product.COCONUT_COUPON]:
                    target_coconut_coupon_position = self.LIMIT[Product.COCONUT_COUPON]
                    if len(coconut_coupon_order_depth.sell_orders) > 0:
                        best_ask = min(coconut_coupon_order_depth.sell_orders.keys())
                        quantity = min(abs(target_coconut_coupon_position - coconut_coupon_position), abs(coconut_coupon_order_depth.sell_orders[best_ask]))
                        traderData["hedged"] = False
    ...
    def run(self, state: TradingState):
        ...
        if Product.COCONUT_COUPON in self.params and Product.COCONUT_COUPON in state.order_depths:
            ...
            coconut_mid_price = (min(coconut_order_depth.buy_orders.keys()) + max(coconut_order_depth.sell_orders.keys())) / 2
            coconut_coupon_mid_price = self.get_coconut_coupon_mid_price(
                coconut_coupon_order_depth,
                traderObject[Product.COCONUT_COUPON]
            )
            tte = self.params[Product.COCONUT_COUPON]['starting_time_to_expiry'] - (state.timestamp) / 1000000 / 250
            volatility = BlackScholes.implied_volatility(coconut_coupon_mid_price, coconut_mid_price, self.params[Product.COCONUT_COUPON]["strike"], tte)
            delta = BlackScholes.delta(coconut_mid_price, self.params[Product.COCONUT_COUPON]["strike"], tte, volatility)

            target_coconut_quantity = -int(delta * coconut_coupon_position)
            if abs(coconut_position) == self.LIMIT[Product.COCONUT] or target_coconut_quantity == coconut_position:
                traderObject[Product.COCONUT_COUPON]["hedged"] = True

            coconut_coupon_orders = self.coconut_coupon_orders(
            ...
            if coconut_coupon_orders != None:
                result[Product.COCONUT_COUPON] = coconut_coupon_orders

                coconut_orders = self.delta_hedge_coconut_coupon_orders(
                ...
                coconut_orders = self.delta_hedge_coconut_position(
                    state.order_depths[Product.COCONUT],
                    coconut_coupon_position,
                    coconut_position,
                    0,
